...
export const useSubmenuPositioning = ({
  ...
  const calculateZIndex = useCallback(
    (baseZIndex = BASE_Z_INDEX): number => baseZIndex + level * Z_INDEX_INCREMENT,
    [level]
  );
  ...
  const getLayerId = useCallback(
    (): string => `${layerIdPrefix}-${level}-${submenuId}`,
    [layerIdPrefix, level, submenuId]
  );
  ...
  const getPositioning = useCallback(
    (anchorElement: HTMLElement): SubmenuPositioning => ({
      anchorElement,
      placement,
