...
export const NestedDropdownProvider: React.FC<NestedDropdownProviderProps> = ({
  ...
  const config = useMemo(() => ({...DEFAULT_NESTED_CONFIG, ...userConfig}), [userConfig]);
    ...
    (id: string, parentId?: string) => {
      ...
      setSubmenus((prev) => {
        ...
        newSubmenus.forEach((submenu, submenuId) => {
          if (submenu.level >= level && submenuId !== id) newSubmenus.delete(submenuId);
        });
        ...
        setSubmenus((prev) => {
          ...
          if (submenu && !submenu.isHovered) {
            ...
            newSubmenus.forEach((childSubmenu, childId) => {
              if (childSubmenu.level > submenu.level) newSubmenus.delete(childId);
            });
   ...
   */
  const closeSubmenusFromLevel = useCallback(
    (level: number) => {
      setSubmenus((prev) => {
        const newSubmenus = new Map(prev);
   ...
   */
  const setSubmenuHover = useCallback(
   ...
   */
  const canOpenSubmenu = useCallback(
  ...
  return <NestedDropdownContext.Provider value={contextValue}>{children}</NestedDropdownContext.Provider>;
};
...
export const useNestedDropdown = (): NestedDropdownContextValue => {
  ...
  if (!context) throw new Error('useNestedDropdown must be used within a NestedDropdownProvider');
...
export const useSubmenu = (id: string) => {
  const {submenus, openSubmenu, closeSubmenu, setSubmenuHover, canOpenSubmenu} = useNestedDropdown();
