  ...
  webpackFinal: async (config) => {
    ...
    return {
      ...
      module: {
        ...config.module,
        rules: [
          ...config.module.rules.filter((r) => !r.test || (!r.test.toString().includes('svg') && !r.test.toString().includes('ts'))),
          ...
          ...custom.module.rules.filter(rule => !rule.test || (!rule.test.toString().includes('ts') && !rule.test.toString().includes('tsx')))
        ]
      }
    };
  }
